    ...
    def tmpl_webcoll_body(self, ln, collection, te_portalbox,
 	...
 	_ = gettext_set_language(ln)
        ...
        body = '''
                ...
                %(searchfor)s
                %(np_portalbox)s
		<div><table class="showupdate" algin="left" style="max-width:600px;float:left;">
		<tr valign="top"><td>
		<table >
		<thead>
		<tr>
		...
		<td>%(showicture)s</td>
                ...
                <table cellspacing="0" cellpadding="0" border="0" class="narrowandfocusonsearchbox" style="max-width:400px;float:left;">
                 
                   <tr valign="top"><td>%(narrowsearch)s</td></tr>
               ''' % {
        ...
        if focuson:
            body += """<tr valign="top"><td>""" + focuson + """</td></tr>"""
        body += """</table></div>
            %(ne_portalbox)s
               </form>""" % {'ne_portalbox' : ne_portalbox}
        return body

    def show_picture(self,ln):
	query = """SELECT DISTINCT d.`value`FROM bib85x as d LEFT JOIN bibrec_bib85x as e 
ON d.id =e.id_bibxxx,bibrec_bibdoc,bibdoc where e.id_bibrec = bibrec_bibdoc.id_bibrec and bibrec_bibdoc.type='picture' 
and bibrec_bibdoc.id_bibdoc=bibdoc.id and d.`value` LIKE '%=icon-700' ORDER BY bibdoc.creation_date DESC
	LIMIT 0,15""" 
	out="""<table width="600" height="800" border="0">
<tbody><tr> """
	i=0
	res = run_sql(query)
	for row in res:
	    i +=1
	    str1=row[0].split('f')
	    str2=str1[0]+"""?ln=zh_CN"""
	    out +="""<td width="33%" height="150"><a href=\""""+str2+"\""+"""><img src=\""""+row[0]+"\""+""" width="150" height="150" /></a></td>"""
	    if i%3 == 0 and i!=15:
		out +="""</tr><tr>"""
	    if i==15:
		out +="""</tr>"""
	...
	out +="""</tbody></table>"""
	out +="""<a style="margin-left:520px;" href="%(siteurl)s/collection/Pictures?ln=zh_CN">更多</a>""" % {'siteurl' : CFG_SITE_URL}
        return out
    ...
    def tmpl_instant_browse(self, aas, ln, recids, more_link=None, grid_layout=False):
        ...
        body = '''<table class="latestadditionsbox" width="1200">'''
        if grid_layout:
            body += '<tr><td><div>'
        for recid in recids:
            if grid_layout:
